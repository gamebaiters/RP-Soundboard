#!/usr/bin/env bash
# ============================================================
#  GameBaiters Soundboard - Uninstaller (Linux)
# ============================================================
set -u
...
echo
echo "  GameBaiters Soundboard - Uninstaller (Linux)"
echo "  ============================================"
echo
...
bak_in="${bak_in:-Y}"
do_backup="${bak_in:0:1}"
do_backup="${do_backup^^}"
echo

# Step 1: kill TS3
if pgrep -x ts3client_linux_amd64 >/dev/null 2>&1 || pgrep -x ts3client_linux_x86 >/dev/null 2>&1; then
    read -rp "  TS3 client is running. Kill it? (y/N): " ans
    if [[ "${ans,,}" == "y" ]]; then
        pkill -9 -x ts3client_linux_amd64 2>/dev/null
        pkill -9 -x ts3client_linux_x86   2>/dev/null
        sleep 2
        echo "  [OK] TS3 terminated."
    else
        echo "  [WARN] Locked files may remain."
    fi
fi

if [[ ! -d "${TS3_BASE}" ]]; then
    echo "  [INFO] No TS3 base at ${TS3_BASE} - nothing to do."
    echo
...
for lib in \
    librp_soundboard_fx.so libsoundboard.so librp_soundboard.so \
    rp_soundboard_fx.so   rp_soundboard.so   soundboard.so      \
    librp_soundboard_fx_linux_amd64.so       librp_soundboard_linux_amd64.so
do
...
# Step 5: logs always; config only if user opted in
echo
echo "  Removing logs..."
